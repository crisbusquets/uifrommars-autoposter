  ...
  async createSchedule(windowName, cronExpression, region, probability) {
    try {
      const result = await this.client.schedules.create({
        ...
        cron: cronExpression,
        body: JSON.stringify({
          payload: { windowName, region, probability },
        }),
        headers: {
      ...
      console.log(`✅ Schedule created for ${windowName}: ${result.scheduleId}`);
  ...
  async setupAllWindows(windows) {
    ...
    for (const [windowName, config] of Object.entries(windows)) {
      try {
        ...
        results.push({ windowName, status: "success", id: result.scheduleId });
      } catch (error) {
